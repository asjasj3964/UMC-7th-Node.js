...
CREATE TABLE member (
    ...
    inactive_at datetime(6) NOT NULL,
    PRIMARY KEY (id),
    FOREIGN KEY (residence_id) REFERENCES map(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
...
CREATE TABLE restaurant (
    ...
    updated_at datetime(6) NOT NULL,
    PRIMARY KEY (id),
    FOREIGN KEY (location_id) REFERENCES map(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
...
- 지도에서 x 좌표, y 좌표를 정해 각 위치를 지정한 map 테이블을 추가해 사용자(member)의 거주지와 음식점(restaurant)의 위치가 참조하도록 하였다. 